...
class LlavaNextForConditionalGeneration(LlavaNextPreTrainedModel):
    ...
    def forward(
        ...
        if (pixel_values is not None) and (labels is not None):
            mask_image_labels = torch.full_like(labels, 0).to(torch.long)
            mask_image_labels[labels != -100] = 1
            
            max_length_inputs = input_ids.size(-1)

            for batch_index in range(len(input_ids)):
                tmp_num = sum(mask_image_labels[batch_index])
                no_zero_first_index = torch.where(mask_image_labels[batch_index] == 0)[-1][-1]
                mask_image_labels[batch_index][(no_zero_first_index-(max_length_inputs-tmp_num)+1):no_zero_first_index+1]=1
